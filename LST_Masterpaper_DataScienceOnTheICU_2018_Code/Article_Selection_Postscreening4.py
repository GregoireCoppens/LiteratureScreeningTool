...
for (dirpath, dirnames, filenames) in walk('./backup4'):
    for f in filenames:
        if f.endswith('.pkl'):
            df_screened_output = df_screened_output.append(
                    pd.read_pickle("./backup4/"+f).reset_index(),
                    ignore_index=True)
...
df.to_pickle("Article_Table_Postscreening4.pkl")
df.to_excel("Article_Table_Postscreening4.xlsx")
df_screened_output.to_pickle("Output_Table_Postscreening4.pkl")
df_screened_output.to_excel("Output_Table_Postscreening4.xlsx")
